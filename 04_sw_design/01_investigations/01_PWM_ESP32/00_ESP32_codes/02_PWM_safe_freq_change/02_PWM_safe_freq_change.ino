...
void change_frequency_safely(int newf) {
  // Trigger synchronization on Timer 0 to align the PWM signal to a known state
  mcpwm_timer_trigger_soft_sync(MCPWM_UNIT_0, MCPWM_TIMER_0);

  // Set the new frequency for Timer 0; the change will take effect starting the next PWM cycle
  mcpwm_set_frequency(MCPWM_UNIT_0, MCPWM_TIMER_0, newf);
}
...
void loop() {
  change_frequency_safely(200000);
  digitalWrite(GPIO_TRIGGER , HIGH);
  delayMicroseconds(135);

  change_frequency_safely(100000);
