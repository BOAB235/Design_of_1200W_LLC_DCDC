...
// First order filter (buckward Euler) 

static int To_filter_us_1 = 10*Ts_us ; // best practice To>>Ts
static float y_prev_fil_1 = 0.0f;
static float alpha_fil_1 = Ts_us / (To_filter_us_1 + Ts_us);

static float first_order_filter_1(float x) {
    float Ts = Ts_us * 1e-6f;
    float To_safe = To;

    float y = y_prev_fil_1 + alpha_fil_1  * (x - y_prev_fil_1);
    y_prev_fil_1 = y;
