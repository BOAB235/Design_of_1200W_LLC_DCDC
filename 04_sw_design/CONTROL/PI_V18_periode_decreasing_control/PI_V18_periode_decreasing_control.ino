...
int max_periode = (int) 720;  //72E3/100=720, 95khz
...
float vout_target=42;
float error_vout;
float Vout_Hysteresis = 0.1;
int TICK_STEP = 2;
int delay_us = 20;

while (1){
if (micros() -timeOld >=delay_us )
{
timeOld = micros(); 
vout = 0.03081f * adc_read(1) - 0.09761f; 
error_vout= vout_target-vout; 

if (error_vout > Vout_Hysteresis) {
// Voltage too low -> Need more gain -> Decrease Freq -> Increase Ticks
Periode += TICK_STEP;
} else if (error_vout < -Vout_Hysteresis) {
// Voltage too high -> Need less gain -> Increase Freq -> Decrease Ticks
Periode -= TICK_STEP;
}
...
//ensure that Periode is x2
Periode = (int) Periode / 2;
Periode = Periode *2 ;

pwm_periode_dutyc_free_glitch(Periode, (int) Periode / 2);
/*
int min_periode = (int) 72E3/150; //=480 , 150 kHz
int max_periode = (int) 72E3/100;  //=720, 100khz
*/
}
}
}