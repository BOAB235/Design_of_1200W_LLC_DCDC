...
/*
int slew_rate_periode(int per){
Periode_old_time_us_new  = micros();
unsigned long delta_Periode_old_time_us = Periode_old_time_us_new -Periode_old_time_us;
if ((1e3*(per -Periode_old))/delta_Periode_old_time_us > slew_rate_hz_by_us ){
  per = Periode_old+(slew_rate_hz_by_us*delta_Periode_old_time_us)/1000 ;
}
else if ((1e3*(per -Periode_old))/delta_Periode_old_time_us < slew_rate_hz_by_us ){
  per = Periode_old-(slew_rate_hz_by_us*delta_Periode_old_time_us)/1000 ;
}
Periode_old=per;
Periode_old_time_us = Periode_old_time_us_new;
return per;
}
*/
...
int max_periode = (int) 72E3/95;  //=757, 95khz
...
int slew_rate_periode(int target_per) {
    unsigned long current_time = micros();
    unsigned long delta_us = current_time - last_time_us;
    
    // Safety for first run or rollover
    if (delta_us > 100000) delta_us = 0; 
    
    // Calculate max allowed change for this time step
    float max_change = SLEW_RATE_TICKS_PER_US * (float)delta_us;
    
    // Calculate difference between current accumulator and target
    float diff = (float)target_per - per_accumulator;
    
    if (abs(diff) <= max_change) {
        // If we are close enough, just go to target
        per_accumulator = (float)target_per;
    } else {
        // Otherwise, move by max_change in the correct direction
        if (diff > 0) {
            per_accumulator += max_change;
        } else {
            per_accumulator -= max_change;
        }
    }
    ...
    // Return as integer for the hardware register
    return (int)per_accumulator;
}


//=========================================== Validation of slew rate=========================
void test_slew_rate_func() {
    // desable HW protection Vout
    digitalWrite( disable_HW_OVP_OUT, HIGH);
    // Enable CH2 + CH2N (CHANGED to CC2E and CC2NE)
    TIM1->CCER |= TIM_CCER_CC2E | TIM_CCER_CC2NE;

    // ENABLE DRIVER & RESET FLIPFLOP
    delayMicroseconds(10);
    digitalWrite(_Dis_drv, HIGH); 
    // RESET FLIPFLOP
    ...
    while(1) {
        // --- RAMP UP (Increase Period / Decrease Frequency) ---
        Periode = 480;
        per_accumulator = 480.0; 
        while (Periode < 710) { // 720 - 10
        // Stop slightly before max to avoid noise toggle
            set_arr_ccr2_synced(Periode, Periode / 2);
            Periode = slew_rate_periode(720); 
        }

        // --- RAMP DOWN (Decrease Period / Increase Frequency) ---
        Periode = 720;
        per_accumulator = 720.0; 
        while (Periode > 490) { // 480 + 10
        // Stop slightly before min
            set_arr_ccr2_synced(Periode, Periode / 2);
            Periode = slew_rate_periode(480); 
        }
    }
}
//===============================================================================================
...
void PWM_Fix_Freq(){
    ...
    int Vin_Min_to_start = 100;
    delay (10);
    while(vin<Vin_Min_to_start )
    {//START AT 100V min 
   ...
   Periode = 686;//105kHz 
    set_arr_ccr2_synced(Periode, Periode / 3);
...
//====whait activation of the ouput voltage=============
// whait activation of the ouput voltage sensor 
// to get error 2.8xx (Vout = 109)
digitalWrite( disable_HW_OVP_OUT, HIGH);
while (vout<95 ){
// vout conversion 
...
float kp = 0.05;  // Low gain for safety
float ki = 25.0;  // Slow integration to prevent overshoot
...
// clamped integration, update only if no depassement

Periode = slew_rate_periode(Periode); 
...
// 6. TIMING 
    time1 = micros();               
    dt = 1e-6 * (float)(time1 - time0); 
    if (dt < 1e-6) dt = 5e-6; // Minimum dt 6µs (around 17us in real)
    time0 = time1;                  
