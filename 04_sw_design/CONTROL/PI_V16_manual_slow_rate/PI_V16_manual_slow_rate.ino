...
/// GLOBAL VARS FOR PI REGULATOR



static inline void __attribute__((always_inline))  reg_loop(){
float vout_target=20;
float error, u ;  
float integ_error = 0.0f;
float integ_error__ = 0.0f;
float dt = 1e-5f;
float kp = 0.05/3;  // Low gain for safety
float ki = 25.0/3;  // Slow integration to prevent overshoot
unsigned long time0, time1; 



// EMA FILTER for vout
static float vout_lowpass = 0.0f; 
float vout_raw;
float alpha = 0.1f; 
...
// 1. Get raw value
vout_raw = 0.03081f * adc_read(1) - 0.09761f;

// 2. Simple Exponential Smoothing (EMA)
// Use alpha = 0.1 for a ~900Hz filter at 17us loop time
vout_lowpass = (alpha * vout_raw) + ((1.0f - alpha) * vout_lowpass);
...
error =  vout_target-vout_lowpass;
// integral
...
while (1){
  ...
  //72e3khz/110kHz=654.5 ticks
  if (Periode>= 654) while(1); // STOP 
  // Time to go down 
  // 654-480=174 ; x50=8.7ms (safe)
}

/*
while(1){
//72e3khz/110kHz=654.5 ticks
Periode = slew_rate_periode(654); 
// force periode x 2 
Periode = (int) Periode/2; 
Periode = (int) Periode*2;
set_arr_ccr2_synced(Periode, (int) Periode / 2);
