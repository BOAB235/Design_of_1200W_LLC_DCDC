...
float T0_der=60;
float T1_der=90;
float P_nom= 1200*1.05;//5% of margin vs specification 
if (Temp < T0_der) return P_nom; 
if (Temp >=  T0_der && Temp <=  T1_der){
//float x_val = 
// =+(D12-60)/(90-60)
//=1200*(1-L12)
 return  P_nom*(1-(Temp-T0_der)/(T1_der-T0_der)); 
}
if (Temp > T1_der) return 0; 
}
///////////////////////////////////////////////////////
float Power_derat_prim_second(){
  ...
  return temp_power_derating( tempS);
}
void loop() {
  ...
  float error , Power ,  Power_derating, Iout, Iin; 
  ...
  while (1) {
    ...
    vin=0.3212 * adc_read(7) - 7.2289;
    Iout = 0.016064*(adc_read(0)-412) ; //se excel file 
    Iin = 0.002008*(adc_read(5)-197) ; //se excel file
    ...
    // PowerDerating
    Power = vout*Iout; 
    Power_derating= Power_derat_prim_second(); // Watt

    // ── SLOW PATH: step only when timer expired + outside deadband ──
    if (((error > DEADBAND)||(Power > Power_derating)) && (micros() - step_timer) >= step_delay) {

      step_timer = micros();  // reset timer BEFORE pwm call (pwm blocks ~7µs)

      if (Power > Power_derating){
        Periode -= TICK_STEP; // Periode decrease → raise freq → less gain → vlow 
      }
      else {
        if (vout < vout_target) {
